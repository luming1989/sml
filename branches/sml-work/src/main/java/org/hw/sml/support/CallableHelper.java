...
public class CallableHelper {
	...
	public static <T> List<T> callresults(int retrys,int max_threads,Callable<T> ...cls){
		...
		for(int i=0;i<times;i++){
			...
			List<T> rt=CallableHelper.callresults(retrys,Arrays.asList(cls).subList(i*max_threads,(i+1)*max_threads>cls.length?cls.length:(i+1)*max_threads).toArray(new Callable[]{}));
			result.addAll(rt);
		}
		return result;
	}
	...
	public static class InnerResult<T>{
		private List<Throwable> es=MapUtils.newArrayList();
		private List<Boolean> flags=MapUtils.newArrayList();
		private List<T> results=MapUtils.newArrayList();
		public void addE(Throwable t){
			es.add(t);
		}
		public void addFlag(boolean f){
			flags.add(f);
		}
		...
		public List<Throwable> getEs() {
			return es;
		}
		public void setEs(List<Throwable> es) {
			this.es = es;
		}
